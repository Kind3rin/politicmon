...
const image = await page.evaluate(async () => {
  ...
  const { newGameState } = await import("/src/game/state.ts"); const { WeeklyCampaignScene } = await import("/src/scenes/WeeklyCampaignScene.ts");
  ...
  stack.push(new WeeklyCampaignScene(stack, input, state, () => undefined)); stack.update(1 / 60); stack.draw(screen); return canvas.toDataURL("image/png");
});
mkdirSync("artifacts/screens", { recursive: true }); writeFileSync("artifacts/screens/p6_weekly_campaign.png", Buffer.from(image.split(",")[1], "base64"));
console.log("salvato screenshot campagna settimanale"); await browser.close();
