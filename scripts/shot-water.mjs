...
const shots = await page.evaluate(async () => {
  ...
  async function world(mapId, x, y, flags) {
    const state = newGameState();
    state.flags["intro-done"] = true; state.flags["hint-casino"] = true;
  ...
  return {
    borgo_pond: await world("borgo", 6, 21),
    route1: await world("route1", 14, 6, ["veh-traghetto"]),
    stretto: await world("stretto", 13, 5, ["veh-traghetto","ponte-beaten"]),
  };
});
