...
import { clipToWidth, drawScreenHeader, GREY, INK, wrapText } from "../ui/widgets";
...
export class SourcesScene implements Scene {
  ...
  draw(screen: Screen): void {
    ...
    for (let row = 0; row < 4; row += 1) {
      ...
      screen.text(`${selected ? "► " : "  "}${clipToWidth(SOURCE_ENTRIES[i].title, 190)}`, 11, y + 5, INK);
    }
    ...
    screen.text(clipToWidth(this.message, 216), 12, 148, "#a46b12");
