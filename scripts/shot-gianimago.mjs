...
const shots = await page.evaluate(async () => {
  ...
  for (const id of ["gianimago", "quasimagiani"]) {
    monsterImage(id); monsterImage(id, true);
  }
  await waitForSprites([
    "mon:gianimago", "mon:gianimago_action", "mon:quasimagiani", "mon:quasimagiani_action"
  ], 4000);
  ...
  dexState.dex.quasimagiani = "caught";
  const dex = new DexScene(stack, input, dexState);
  dex.index = DEX_ORDER.indexOf("quasimagiani"); dex.detail = true;

  const battleState = newGameState();
  battleState.party = [createMonster("quasimagiani", 38)];
  const battle = new BattleScene(stack, input, {
    state: battleState, foeTeam: [createMonster("gianimago", 31)], onEnd: () => {}
  });
  battle.queue = []; battle.mode = "menu"; battle.introT = 1; battle.firstSeenBanner = 0;
  battle.fx.lungeT.player = 0.2;

  const evolution = new EvolutionScene(stack, input, "gianimago", "quasimagiani", () => {});
  evolution.phase = 3; evolution.time = 1;
  return {
    images: { evolution: render(evolution), dex: render(dex), battle: render(battle) },
    debug: {
      front: [spriteStatus("mon:quasimagiani"), monsterImage("quasimagiani")?.naturalWidth ?? 0],
      action: [spriteStatus("mon:quasimagiani_action"), monsterImage("quasimagiani", true)?.naturalWidth ?? 0]
    }
  };
});
...
for (const [name, dataUrl] of Object.entries(shots.images)) {
  const path = `artifacts/screens/gianimago_${name}.png`;
