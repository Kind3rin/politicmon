// Screenshot delle mappe con i nuovi NPC ambientali (Eurotown, Capitale).
...
const shots = await page.evaluate(async () => {
  ...
  function shotMap(mapId, x, y, hintFlag) {
    const state = newGameState();
    state.flags["intro-done"] = true;
    if (hintFlag) state.flags[hintFlag] = true; // salta l'hint per vedere la mappa
  ...
  return {
    euro: shotMap("eurotown", 12, 11, null),
    cap: shotMap("capitale", 19, 18, "hint-casino")
  };
});
...
save("map_eurotown", shots.euro);
save("map_capitale", shots.cap);
console.log("salvati map_eurotown, map_capitale");
await browser.close();
