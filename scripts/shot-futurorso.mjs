...
const shots = await page.evaluate(async () => {
  ...
  monsterImage("vannaccix"); monsterImage("futurorso"); monsterImage("futurorso", true);
  await waitForSprites(["mon:vannaccix", "mon:futurorso", "mon:futurorso_action"], 4000);
  ...
  dexState.dex.futurorso = "caught";
  const dex = new DexScene(stack, input, dexState);
  dex.index = DEX_ORDER.indexOf("futurorso");
  dex.detail = true;

  const battleState = newGameState();
  battleState.party = [createMonster("futurorso", 40)];
  const battle = new BattleScene(stack, input, {
    state: battleState,
    foeTeam: [createMonster("generorso", 40)],
    onEnd: () => {}
  });
  battle.queue = [];
  battle.mode = "menu";
  ...
  const evolution = new EvolutionScene(stack, input, "vannaccix", "futurorso", () => {});
  evolution.phase = 3;
  evolution.time = 1;
  ...
  return {
    ...
    debug: {
      futurorso: [spriteStatus("mon:futurorso"), monsterImage("futurorso")?.naturalWidth ?? 0],
      futurorsoAction: [spriteStatus("mon:futurorso_action"), monsterImage("futurorso", true)?.naturalWidth ?? 0]
    }
  };
});
...
for (const [name, dataUrl] of Object.entries(shots.images)) {
  const path = `artifacts/screens/futurorso_${name}.png`;
