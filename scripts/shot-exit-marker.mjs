// Verifica marker USCITA: presente in grotta (porta invisibile), assente nel
// bar (porta rossa visibile).
...
const shots = await page.evaluate(async () => {
  ...
  return {
    grotta: shot("grotta1", 8, 11),   // vicino all'uscita (7/8,13)
    bar: shot("bar-medio", 5, 5)       // interno bar (porta rossa visibile)
  };
});
function save(n, d){ writeFileSync(`artifacts/screens/${n}.png`, Buffer.from(d.slice("data:image/png;base64,".length),"base64")); }
save("exit_grotta", shots.grotta);
save("exit_bar", shots.bar);
console.log("salvati exit_grotta, exit_bar");
await browser.close();
