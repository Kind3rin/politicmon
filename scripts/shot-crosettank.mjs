...
const shots = await page.evaluate(async () => {
  ...
  monsterImage("crosettank"); monsterImage("crosettank", true);
  await waitForSprites(["mon:crosettank", "mon:crosettank_action"], 4000);
  const render = (scene) => {
  ...
  const dexState = newGameState(); dexState.dex.crosettank = "caught";
  const dex = new DexScene(stack, input, dexState);
  dex.index = DEX_ORDER.indexOf("crosettank"); dex.detail = true;
  const battleState = newGameState(); battleState.party = [createMonster("crosettank", 38)];
  const battle = new BattleScene(stack, input, {
    state: battleState, foeTeam: [createMonster("quasimagiani", 38)], onEnd: () => {}
  });
  ...
  return {
    ...
    debug: {
      front: [spriteStatus("mon:crosettank"), monsterImage("crosettank")?.naturalWidth ?? 0],
      action: [spriteStatus("mon:crosettank_action"), monsterImage("crosettank", true)?.naturalWidth ?? 0]
    }
  };
});
...
for (const [name, dataUrl] of Object.entries(shots.images)) {
  const path = `artifacts/screens/crosettank_${name}.png`;
